...
const TermsManagement: React.FC = () => {
    ...
    const handleSave = async () => {
        setIsSaving(true);
        try {
            await api.post('/content/terms', { content, seo });
            showToast('success', 'Terms & Conditions Saved', 'Terms page content updated.');
        } catch (err) {
            console.error("Error saving terms data", err);
            showToast('error', 'Update Failed', 'Failed to update Terms & Conditions Page.');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 pb-10 max-w-7xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--color-16)]">Terms & Conditions Management</h1>
                    <p className="text-[var(--color-20)]">Manage the legal terms and conditions of your platform.</p>
                </div>
                <button
                    onClick={handleSave}
                    disabled={isSaving}
                            ...
                            <div className="flex flex-col gap-1 max-w-xl">
                                <div className="flex items-center gap-2 text-sm text-[#202124]">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[12px] text-[#4d5156] leading-tight">https://avenstek.com › terms-and-conditions</span>
                                    </div>
                                </div>
                                <h3 className="text-[20px] text-[#1a0dab] font-medium leading-tight hover:underline cursor-pointer mt-1">
                                    {seo.title || "Terms & Conditions | Avenstek Solutions"}
                                </h3>
                                <p className="text-[14px] text-[#4d5156] leading-relaxed mt-1 line-clamp-2">
                                    {seo.description || "Read our terms and conditions to understand the rules and guidelines for using Avenstek Solutions platform."}
                                </p>
                            </div>
                        </div>
